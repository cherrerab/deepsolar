...
#------------------------------------------------------------------------------
def solar_azimuth(timestamp, lat=-33.45775, lon=70.66466111):
    """
    -> float
    
    Calcula el azimuth de la posición de sol en el timestamp entregado.
    ...
    :returns:
        float del azimuth en radianes de la posición del sol.
    """
...
#        return 0.0
    ...
    cos_theta = (sin(delta)*sin(lat) + cos(delta)*cos(lat)*cos(w))
    cos_theta = 0.0 if cos_theta < 0.0 else cos_theta
    
    # ángulo de elevación
    alpha = pi/2.0 - acos(cos_theta)
    
    # azimuth
    zeta = asin( (cos(delta)*sin(w))/cos(alpha) )
