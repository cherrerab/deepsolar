...
from deepsolar._tools import ext_irradiance, ext_irradiation, get_timestep

from deepsolar import (validate_date, get_date_index)
...
def plot_2D_radiation_data(database, unit='', **kargs):
    ...
    days = [initial_day + timedelta(days=delta_days*x) for x in range(num_ticks + 1)]
    ...
    plt.xticks( range(0, (num_ticks+1)*delta_days, delta_days), day_ticks )
...
def plot_1D_radiation_data(database, colname, start_date, stop_date, 
    ...
    plt.plot(X, Y*scale_factor, c='k', ls='-', lw=0.8, label='Global en plano horizontal')
...
def plot_error_dist(Y_true, Y_pred, pred_labels, horizons, **kargs): 
    ...
    colors = ['lightgrey', 'black', 'grey', 'grey']
    line_styles = [None, '-', '--', '-.']
